...
| P1        | Timski fit kandidata product/report contract spec | Mock-safe generation shell implemented / no provider/renderer/worker | Relacijski report / Candidate-team fit | Next slice: provider seam or read-only display helper planning, only after explicit approval. |
