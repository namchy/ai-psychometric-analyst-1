...
> **Update — controlled Golden Demo foundation writer implementation:** dodat je transaction-safe `create_golden_demo_foundation_v1()` RPC migration i zaseban operator CLI. RPC resolvea postojeću Partner Plus organizaciju, validira canonical identity/team/membership graph prije prvog inserta, kreira samo missing participants/teams/memberships i vraća `EXACT_MATCH` postcondition; organization i assessment artefakti nisu write scope. CLI je default read-only preflight, a apply zahtijeva `--apply --confirm GOLDEN_DEMO_FOUNDATION`; live foundation inspector/RPC nisu pokrenuti. Offline testovi pokrivaju missing-only `EMPTY`/`PARTIAL` plan, conflict rejection, atomic RPC failure, idempotent EXACT no-op i CLI guards. Foundation write ostaje neodobren dok operator zasebno ne uradi reviewed live preflight i eksplicitni confirmed apply.
