...
| P1        | Cross-report validator boundary and report ownership audit | Završeno za SAFRAN/MWMS current boundary krug; composite HR boundary reduction završen; shared single-test HR AI input capture inspector dodat za SAFRAN/MWMS/IPIP; SAFRAN production-equivalent capture + confirmed dry-run PASS; SAFRAN data-only shadow diagnostics završeni; SAFRAN HR structured score/reference anchors završen; MWMS production-equivalent capture + confirmed dry-run PASS; potvrđena AI report ownership / validator boundary doctrine; MWMS general-envelope-only i data-only shadow diagnostics završeni; MWMS HR production gate prebačen na data-only blocking validation | Report architecture / Validator boundary audit | MWMS HR ostaje prvi production lane prebačen na data-only blocking validation. SAFRAN je sada spremniji za zaseban production gate switch plan, ali production switch još nije urađen. Dalji boundary work treba i dalje pratiti doktrinu: structure/data validation only, bez app-side prose authority, bez big-bang refactora i bez brisanja starih validatora naslijepo. |
