...
> **Current GDT-01 Team Dynamics persistence status — 22.07.2026:** `GDT01_CORRECTIVE_MIGRATION_APPLIED_AND_VALIDATED`. Commit `881d4f45` is the latest source commit. Migrations `20260721143000_create_gdt01_team_dynamics_seed_rpc.sql` and `20260721150000_fix_gdt01_membership_resolution_update.sql` are applied. The corrective migration removed the hosted `pg_safeupdate` error `UPDATE requires a WHERE clause`; membership update is correlated through `participant_id`. The security contract remains `SECURITY DEFINER`, empty `search_path`, and execute only for `service_role`.
>
> Foundation is `EXACT_MATCH`: 24 participants, 4 teams and 24 active memberships. GDT-01 remains `EMPTY`, `writerEligible: true`. Assignment, wrappers, attempts, responses, selections, scores, aggregations and reports remain 0. The corrective migration did not seed data.
>
> **Next step — lean controlled GDT-01 seed:** run one existing apply command, then perform a read-only footprint check for `1 assignment / 6 wrappers / 6 attempts / 288 responses / 72 physical selections / 324 logical selections`. Do not repeat already-proven source, ACL or migration audits. Scoring, aggregation, Team Fit, AI and evaluation remain later separate steps.
...
**Naredni očekivani milestone:** Review report-generation/AI isolation contracta i priprema zasebnog, eksplicitno odobrenog report operatora. Za Golden Demo foundation lane, posljednji source commit je `881d4f45`, a potvrđena corrective migration granica je zatvorena. Sljedeći persistence korak je lean kontrolisani GDT-01 seed i read-only footprint provjera; completion, scoring, aggregation, Team Fit, AI i evaluacija ostaju zasebni kasniji koraci. Scoring apply se ne ponavlja; `fixtureWriterState=CONFLICT` nakon scoringa je namjerna zaštita od ponovnog seedovanja, dok `fixtureCompatibilityState=EXACT_MATCH` i `scoringState=SCORED_EXACT` označavaju validan post-scoring fixture.

**Aktivni preflight:** Team Fit Golden Demo audit ostaje zaseban relacijski-module slice. Preliminarni repo dokaz potvrđuje da Team Fit nije `public.tests` instrument niti četvrti standard-battery test: `team_fit_report_v1` je opcionalni HR-interni report nad kandidatovim Composite deterministic inputom i Team Dynamics agregatom. Ne smije blokirati standard battery, individualne izvještaje ni stvarati PARTIAL/error kada nije uključen. Identity/team foundation je `EXACT_MATCH`; GDT-01 corrective migration je primijenjena i validirana, target graph je `EMPTY`, a writer je eligible. Team Fit report ostaje kasniji korak prema `docs/deep-profile-golden-demo-team-fit-preflight.md`.

**GDT-01 Team Dynamics persistence status:** identity/team foundation je završen live `EXACT_MATCH` (`24 participants`, `4 teams`, `24 active memberships`). Posljednji source commit je `881d4f45`; obje GDT-01 migracije su primijenjene i validirane, uz zadržan `SECURITY DEFINER` / prazan `search_path` / `service_role`-only execute contract. GDT-01 je i dalje `EMPTY`, `writerEligible: true`, sa nulom assignmenta, wrappera, attemptova, responses, selections, scoreova, agregacija i reporta; corrective migration nije pokrenula seed. Sljedeći korak je jedna lean seed apply komanda i read-only footprint provjera; standard battery i GD-001 ostaju nepromijenjeni.
