...
**Decision note — Team Dynamics aggregation persistence lifecycle hardening:**
...
**Sljedeći korak:**  
Team Dynamics stale/recalculation ownership helper slice: uvesti jedan server-only lifecycle helper koji eksplicitno procjenjuje da li je persisted aggregation snapshot potencijalno stale u odnosu na source member score snapshotove i kontrolisano odlučuje da li treba recalculation, bez UI prikaza, bez report orchestration-a, bez attempt_reports, bez assessment_reports, bez AI/report generation-a i bez Team Fit outputa.
