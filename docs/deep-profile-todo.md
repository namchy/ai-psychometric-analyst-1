...
| P1        | Cross-report validator boundary and report ownership audit | Planirano / read-only audit | Report architecture / Validator boundary audit | Prvo dokazati da validatori ne masakriraju dobar AI output; sljedeće: `Composite HR raw inspector / validator-on vs validator-off diagnostic`, zatim isti slice za SAFRAN HR pa MWMS HR. IPIP HR se ne dira jer je već prošao quality/hardening krug. |
...
- `Sljedeće: Composite HR raw inspector / validator boundary diagnostic, dev-only i bez DB write-a, bez report regenerationa i bez promjene production flow-a; capture raw AI output, canonicalized output, validator result i failure reasons, pa compare validator-on vs validator-off behavior bez slabljenja safety/source/shape validation.`
