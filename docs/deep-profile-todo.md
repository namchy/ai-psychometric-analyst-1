...
**Aktivni sljedeći task — `Team Fit V2 active runtime cutover`.** Potrebno je objediniti version-aware display/read-model slice i minimalni action cutover: postojeći URL mora dispatchovati V2 na novi `TeamFitReportV2View`, a postojeći V1 samo na read-only legacy renderer; novi creation/action path queuea V2 i process action koristi injected-provider V2 processor. Novo user-facing V1 kreiranje/procesiranje mora biti onemogućeno. Slice je offline: bez migration applyja, live DB writea, stvarnog OpenAI poziva, schedulera, mass regenerationa ili rollouta drugih kandidata/timova.

**Kasniji guarded task — `Team Fit V2 migration activation and canonical GD-001 × GDT-01 smoke`.** Zahtijeva zasebno eksplicitno odobrenje za read-only remote schema/history preflight, migration apply, live DB write, tačno jedan novi V2 report, tačno jedan OpenAI poziv, persisted contract/evidence/lineage provjeru i browser smoke, uz dokaz da postojeći V1 report nije izmijenjen. Ovaj todo sync ne odobrava migration apply ni smoke.
...
| P1        | Golden Demo Cohort and AI report calibration | V2 content prihvaćen; lifecycle i input/processor foundation commitovani; persisted V2 smoke pending | Demo data / Report QA / Calibration | GD-001 scoring i canonical GD-001 × GDT-01 V2 preview su potvrđeni. Sljedeći korak je offline `Team Fit V2 active runtime cutover`; tek nakon njega zasebno odobren migration activation i canonical persisted smoke. |
...
| P1        | Team Fit clean fixture pair planning slice | Završeno / historical plan superseded canonicalnim GD-001 × GDT-01 previewom | Relacijski report / Candidate-team fit | Ne otvarati novi fixture-prep ili runtime-planning slice: canonical source par i plan su potvrđeni; aktivni sljedeći task je offline V2 active runtime cutover. |
| P1        | Team Fit clean candidate fixture-prep operator script | Završeno za prethodni dev-only operator plan; superseded canonicalnim preview source parom | Relacijski report / Candidate-team fit | Ne pokretati confirmed fixture write; sljedeći live write pripada samo zasebno odobrenom V2 migration activation + canonical smoke tasku nakon offline cutovera. |
...
| P1        | Timski fit kandidata product/report contract spec | V2 content accepted; lifecycle foundation i version-aware input/V2 processor foundation commitovani; V2 active-cutover odluka zaključana. Migracija postoji, ali nije primijenjena; display/action aktivacija i persisted smoke su pending. V1 je zamrznut kao read-only compatibility. | Relacijski report / Candidate-team fit | Aktivni task: `Team Fit V2 active runtime cutover` — V2 loader/view i novi V2 creation/process action path, uz legacy V1 read-only prikaz. Nakon toga zasebno odobren `Team Fit V2 migration activation and canonical GD-001 × GDT-01 smoke`. |
...
- Aktivni sljedeći task je `Team Fit V2 active runtime cutover`. Kasniji guarded task je `Team Fit V2 migration activation and canonical GD-001 × GDT-01 smoke`; oba su odvojena, a ovaj sync ne odobrava live operacije.
...
**Status:** V2 content accepted; versioned lifecycle i version-aware input/V2 processor foundation commitovani; V2 active runtime cutover je sljedeći task; migracija nije primijenjena, display/action aktivacija i canonical persisted smoke su pending; V1 je read-only compatibility
...
- Sljedeći task je `Team Fit V2 active runtime cutover`: V2 display loader/read model, `TeamFitReportV2View`, isti route URL, novi V2 creation/process action path i onemogućeno novo user-facing V1 kreiranje/procesiranje; postojeći V1 ostaje read-only legacy prikaz.
- Cutover se implementira offline sa injected/mock providerom, bez migration applyja, live DB writea, stvarnog OpenAI poziva, schedulera, mass regenerationa ili drugih kandidata/timova.
- Nakon offline cutovera otvoriti zasebno odobren `Team Fit V2 migration activation and canonical GD-001 × GDT-01 smoke`: remote read-only schema/history preflight, migration apply, jedan novi V2 red, jedan OpenAI poziv, persisted validacije i browser smoke uz nepromijenjen postojeći V1 report.
