...
> **Current GDT-01 persistence status — superseding the historical inspector notes below:** `GDT01_CORRECTIVE_MIGRATION_APPLIED_AND_VALIDATED`. Commit `881d4f45` is the latest source commit. Migrations `20260721143000_create_gdt01_team_dynamics_seed_rpc.sql` and `20260721150000_fix_gdt01_membership_resolution_update.sql` are applied. The corrective migration removed the hosted `pg_safeupdate` error `UPDATE requires a WHERE clause`; membership update is correlated through `participant_id`. Security remains `SECURITY DEFINER`, empty `search_path`, execute only for `service_role`. Foundation is `EXACT_MATCH` with 24 participants, 4 teams and 24 active memberships; GDT-01 is `EMPTY`, `writerEligible: true`, and all target persistence/scoring/report counts remain 0. The corrective migration did not seed data.
>
> **Next step:** one existing GDT-01 apply command, followed by a read-only footprint check: `1 assignment / 6 wrappers / 6 attempts / 288 responses / 72 physical selections / 324 logical selections`. Source, ACL and migration audits are already proven and are not repeated; scoring, aggregation, Team Fit, AI and evaluation remain later separate steps.
...
- GDT-01 Team Dynamics canonical explicit fixture, runtime snapshot and source persistence package are complete; the latest source commit is `881d4f45`. The server-owned SQL manifest is the authored-answer content boundary and the runtime checksum is not an authored-answer digest. Corrective migration validation is complete; GDT-01 remains `EMPTY` and unseeded, so the next step is the controlled seed plus read-only footprint check.
