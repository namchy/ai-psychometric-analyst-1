...
> **Current GDT-01 persistence status — superseding the historical inspector notes below:** `GDT01_SEED_APPLIED_AND_EXACT_MATCH_VERIFIED`. The validated migration/source baseline was commit `881d4f45`. Migrations `20260721143000_create_gdt01_team_dynamics_seed_rpc.sql` and `20260721150000_fix_gdt01_membership_resolution_update.sql` are applied. The corrective migration removed the hosted `pg_safeupdate` error `UPDATE requires a WHERE clause`; membership update is correlated through `participant_id`. Security remains `SECURITY DEFINER`, empty `search_path`, execute only for `service_role`. Foundation is `EXACT_MATCH` with 24 participants, 4 teams and 24 active memberships. The earlier single seed apply is confirmed with `1 assignment / 6 wrappers / 6 attempts / 288 responses / 72 physical selections / 324 logical selections`; writer read-only state is `EXACT_MATCH` no-op with `rpcAllowed: false`. There are no scores, aggregation snapshots or reports. The batched `response_selections` read removed instability from the large `.in(...)` request.
>
> **Next step:** lean controlled production member scoring, without aggregation in the same task. Source, ACL and migration audits are already proven and are not repeated; Team Fit, AI and evaluation remain later separate steps.
