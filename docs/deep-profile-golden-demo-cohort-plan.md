...
> GDT-01 canonical explicit answers are the authored persistence input; the separately named recipe is provenance-only. The frozen contract has six members, 288 physical response rows, 72 physical SJT `response_selections` rows and 324 logical option selections, and remains deterministic-only (`ai_prompt_calibration_allowed=false`). The corrective migrations are applied and validated; the earlier single seed apply is confirmed `EXACT_MATCH`, and production member scoring is now verified `SCORED_EXACT` with 6 persisted score snapshots and 48 verified entries. Aggregation, report and OpenAI steps have not run.
...
> **Current GDT-01 persistence status — superseding the historical inspector notes below:** `GDT01_MEMBER_SCORING_COMPLETED_AND_EXACT_MATCH_VERIFIED`. The validated migration/source baseline was commit `881d4f45`. Migrations `20260721143000_create_gdt01_team_dynamics_seed_rpc.sql` and `20260721150000_fix_gdt01_membership_resolution_update.sql` are applied. The corrective migration removed the hosted `pg_safeupdate` error `UPDATE requires a WHERE clause`; membership update is correlated through `participant_id`. Security remains `SECURITY DEFINER`, empty `search_path`, execute only for `service_role`. Foundation is `EXACT_MATCH` with 24 participants, 4 teams and 24 active memberships. The earlier single seed apply is confirmed with `1 assignment / 6 wrappers / 6 attempts / 288 responses / 72 physical selections / 324 logical selections`; production member scoring is `SCORED_EXACT` with 6 persisted score snapshots and 48 verified entries; aggregation snapshots and reports remain 0. The batched `response_selections` read removed instability from the large `.in(...)` request.
>
> **Next step:** lean controlled Team Dynamics aggregation over the six verified member scores, without Team Fit/report generation in the same task. Source, ACL and migration audits are already proven and are not repeated; AI and evaluation remain later separate steps.

> **Historical inspector evidence (superseded):** the first/second/third reviewed inspector blocks below retain earlier runtime and foundation states and must not be read as the current GDT-01 status.
...
- GDT-01 Team Dynamics canonical explicit fixture, runtime snapshot and source persistence package are complete; the validated migration/source baseline is `881d4f45`. The server-owned SQL manifest is the authored-answer content boundary and the runtime checksum is not an authored-answer digest. Corrective migration validation, the earlier single seed and production member scoring are complete; the read-only scoring operator confirms `SCORED_EXACT`, 6 persisted score snapshots and 48 verified entries over the canonical `1 / 6 / 6 / 288 / 72 / 324` footprint. The next step is controlled Team Dynamics aggregation without Team Fit/report generation in the same task.
