...
> GDT-01 canonical explicit answers are the future DB writer input; the separately named recipe is provenance-only. The frozen contract has six members, 288 response objects and 324 selections, and remains deterministic-only (`ai_prompt_calibration_allowed=false`). No live DB operation, report or OpenAI call has occurred.
