...
async function createReportReadySmokeFixture(input) {
  ...
  const organizationSlug = `td-openai-db-smoke-${token}`;
  const fixtureName = `TD OpenAI DB Smoke ${token}`;
  ...
  const participantPayload = Array.from({ length: 4 }, (_, index) => ({
    ...
    user_id: null,
    email: `td-openai-db-member-${index + 1}-${token}@example.test`,
    full_name: `TD OpenAI DB Member ${index + 1}`,
  ...
  const { data: teamRows, error: teamError } = await supabase
    ...
    .insert({
      ...
      description: "Cleanup-safe Team Dynamics Executive Overview OpenAI DB smoke fixture",
...
function summarizeControlledFailure(processed) {
  const summary = {
    operation: processed.operation,
    marker: processed.marker,
    reason: processed.reason,
    providerCode: processed.provider?.ok === false ? processed.provider.code : null,
    failureErrorMessage: processed.final?.ok === true ? processed.final.failure.errorMessage : null,
    validationEvidence:
      processed.provider?.ok === false && Array.isArray(processed.provider.validationErrors)
        ? processed.provider.validationErrors.slice(0, 5)
        : null,
  };

  if (Array.isArray(summary.validationEvidence) && summary.validationEvidence.length === 0) {
    summary.validationEvidence = null;
  }
...
async function main() {
    ...
    console.log(
      JSON.stringify(
        buildSkipResult("Missing OPENAI_API_KEY or AI_REPORT_MODEL.", {
          verified: [
            "runtime Supabase env detected",
            "skip is based on missing OpenAI env, not assumed success",
          ],
        }),
        null,
        2,
  ...
  const keepSmokeData = process.env.KEEP_TD_OPENAI_SMOKE_DATA === "1";
  ...
  try {
    ...
    if (!processed.ok) {
      if (
        processed.operation === "provider_failed" ||
        processed.operation === "snapshot_invalid" ||
        processed.operation === "input_snapshot_missing" ||
        processed.operation === "input_snapshot_invalid"
      ) {
        console.log(
          JSON.stringify(
            buildControlledFailureResult(
              "Team Dynamics Executive Overview OpenAI DB smoke reached a controlled processor failure.",
              {
                verified: [
                  "saved selection exists or was created",
                  "queued team_assessment_reports row was persisted",
                  "processor executed against a real OpenAI provider path",
      ...
      throw new Error(
        `Unexpected Team Dynamics Executive Overview OpenAI processor failure: ${processed.operation} | ${processed.reason}`,
      );
    }
    ...
    assert.equal(processed.report.errorMessage, null);
      ...
      JSON.stringify(
        {
          ...
          verified: [
            ...
            "provider-backed processor completed queued -> processing -> ready through a real OpenAI provider path",
            "input_snapshot was persisted on the report row",
            "report_snapshot was persisted on the report row",
          ...
          fixture: {
            ...
            teamAssessmentReportId: queued.report.id,
            includedMemberCount: includedParticipantIds.length,
            preservedForInspection: keepSmokeData,
          },
          openAi: {
            model: process.env.AI_REPORT_MODEL,
            providerCode: processed.provider.code,
            generatedAt: processed.provider.generatedAt,
          },
          counts: {
            before: beforeCounts,
            after: afterCounts,
    ...
    try {
      ...
      console.warn(
        cleanupError instanceof Error
          ? `Team Dynamics Executive Overview OpenAI DB smoke cleanup warning: ${cleanupError.message}`
          : `Team Dynamics Executive Overview OpenAI DB smoke cleanup warning: ${String(cleanupError)}`,
      );
    }
  }
}
...
main().catch((error) => {
  if (isSchemaCacheMiss(error)) {
    console.log(
      JSON.stringify(
        buildSkipResult(
          "Runtime Supabase schema cache does not expose the required Team Dynamics report lane tables.",
          {
            runtimeError: {
              code: error.code ?? null,
              message: error.message,
