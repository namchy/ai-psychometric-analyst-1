...
assert.doesNotMatch(queueListSource, /Retry|Pokušaj ponovo|Ponovi obradu/i);
...
assert.doesNotMatch(failedMarkup, /Otvori izvještaj/);
assert.doesNotMatch(failedMarkup, /Retry|Pokušaj ponovo|Ponovi obradu/i);
  ...
  JSON.stringify(
    {
      ...
      verified: [
        ...
        "failed report shows Nije uspjesno kreiran without retry UI",
        "queue list uses server action boundary through a dedicated UI action component",
