...
function buildRunner({ failCandidate = null } = {}) {
  const calls = [];
  ...
  async function runner(request) {
    ...
    if (scriptName === "write-gd-001-db-fixture.cjs") {
      ...
      return {
        exitCode: 0,
        stdout: JSON.stringify({
          state: fixtureApplied.has(candidateId) ? "EXACT_MATCH" : "EMPTY",
          responses: { resolved: 184 },
          blockers: [],
    ...
    if (scriptName === "score-gd-001.cjs") {
      ...
      return {
        exitCode: 0,
        stdout: JSON.stringify({
          scoringState: scored.has(candidateId) ? "SCORED_EXACT" : "UNSCORED_EXACT",
          expectedScoreVerification: { ok: true, matched: 47, expected: 47 },
          writesPerformed: false,
        }),
        stderr: "",
      };
