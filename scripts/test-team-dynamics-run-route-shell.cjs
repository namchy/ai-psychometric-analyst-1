...
assert.match(routeSource, /shouldRenderActiveMixedRuntimePreview/);
assert.match(routeSource, /isMixedRuntimePreview && handoff\.isRunnableShellState/);
...
assert.match(routeSource, /Odgovori su spremljeni i procjena je oznacena kao zavrsena\./);
assert.match(routeSource, /Izvjestaji i[\s\S]*dalja obrada bice omoguceni kroz zaseban korak\./);
...
assert.match(componentSource, /wrapperStatus: "invited" \| "started" \| "completed" \| "expired"/);
...
assert.match(componentSource, /Procjena je završena/);
assert.match(componentSource, /Pitanja, spremanje odgovora i completion CTA više nisu dostupni/);
