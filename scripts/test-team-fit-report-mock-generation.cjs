...
assert.doesNotMatch(processorSource, /OpenAI|provider|renderer|worker|scheduler/i);
...
async function main() {
  ...
  assert.equal(typeof invalidSupabase.state.team_fit_reports[0].error_message, "string");

  const refreshedTodoSource = fs.readFileSync(todoPath, "utf8");
  assert.match(refreshedTodoSource, /Mock-safe generation shell implemented \/ no provider\/renderer\/worker/);
