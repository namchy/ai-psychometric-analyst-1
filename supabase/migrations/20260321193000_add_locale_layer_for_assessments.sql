...
create table if not exists public.question_localizations (
  ...
  locale text not null check (locale in ('bs', 'hr')),
  text text not null,
...
create table if not exists public.answer_option_localizations (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (answer_option_id, locale)
);
...
create table if not exists public.prompt_version_localizations (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (prompt_version_id, locale)
);

create index if not exists idx_attempts_locale
  on public.attempts (locale);

create index if not exists idx_question_localizations_question_id
  on public.question_localizations (question_id);

create index if not exists idx_question_localizations_locale
  on public.question_localizations (locale);

create index if not exists idx_answer_option_localizations_answer_option_id
  on public.answer_option_localizations (answer_option_id);

create index if not exists idx_answer_option_localizations_locale
  on public.answer_option_localizations (locale);

create index if not exists idx_prompt_version_localizations_prompt_version_id
  on public.prompt_version_localizations (prompt_version_id);

create index if not exists idx_prompt_version_localizations_locale
  on public.prompt_version_localizations (locale);

create or replace function public.set_question_localizations_updated_at()
returns trigger
language plpgsql
as $$
begin
...
create or replace function public.set_answer_option_localizations_updated_at()
returns trigger
language plpgsql
as $$
begin
...
create or replace function public.set_prompt_version_localizations_updated_at()
returns trigger
language plpgsql
as $$
begin
...
drop trigger if exists set_question_localizations_updated_at on public.question_localizations;

create trigger set_question_localizations_updated_at
before update on public.question_localizations
for each row
execute function public.set_question_localizations_updated_at();

drop trigger if exists set_answer_option_localizations_updated_at on public.answer_option_localizations;

create trigger set_answer_option_localizations_updated_at
before update on public.answer_option_localizations
for each row
execute function public.set_answer_option_localizations_updated_at();

drop trigger if exists set_prompt_version_localizations_updated_at on public.prompt_version_localizations;

create trigger set_prompt_version_localizations_updated_at
before update on public.prompt_version_localizations
for each row
execute function public.set_prompt_version_localizations_updated_at();
...
grant select, insert, update, delete on table public.question_localizations to service_role;
grant select, insert, update, delete on table public.answer_option_localizations to service_role;
grant select, insert, update, delete on table public.prompt_version_localizations to service_role;
