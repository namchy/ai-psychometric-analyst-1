-- Forward-only corrective migration for canonical Golden Demo legacy addressing compatibility.
-- The function remains EMPTY-only: it never repairs, upserts, overwrites,
-- scores, reports, or touches the separate Team Dynamics lane.
create or replace function public.create_golden_demo_standard_battery_fixture_v2(p_fixture jsonb)
returns jsonb
language plpgsql
security definer
set search_path = ''
...
declare
  ...
  v_expected_addressing_form text;
  ...
  v_existing_participant_type text;
  v_existing_status text;
  v_existing_addressing_form text;
begin
  ...
  v_candidate_id := p_fixture ->> 'candidate_id';
  if p_fixture ->> 'schema_version' is distinct from 'gd_db_fixture_v1'
    or v_candidate_id is null
    or v_candidate_id not in ('GD-001', 'GD-002', 'GD-003', 'GD-004', 'GD-005')
    or p_fixture ->> 'organization_name' is distinct from 'Partner Plus d.o.o., Mikrokreditna organizacija'
  then
    raise exception 'GD_FIXTURE_INVALID: candidate-aware fixture identity is not allowed.';
  end if;

  if v_candidate_id = 'GD-001' then
    v_expected_email := 'amel.kovacevic@partnerplus.ba';
    v_expected_display_name := 'Amel Kovačević';
    v_expected_addressing_form := 'masculine';
  elsif v_candidate_id = 'GD-002' then
    v_expected_email := 'natasa.rapaic@partnerplus.ba';
    v_expected_display_name := 'Nataša Rapaić';
    v_expected_addressing_form := 'feminine';
  elsif v_candidate_id = 'GD-003' then
    v_expected_email := 'vladimir.lucic@partnerplus.ba';
    v_expected_display_name := 'Vladimir Lučić';
    v_expected_addressing_form := 'masculine';
  elsif v_candidate_id = 'GD-004' then
    v_expected_email := 'natali.delic@partnerplus.ba';
    v_expected_display_name := 'Natali Delić';
    v_expected_addressing_form := 'feminine';
  ...
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended('golden-demo:partner-plus:' || v_candidate_id, 0)
  );
  ...
  if pg_catalog.jsonb_typeof(p_fixture -> 'participant') is distinct from 'object'
    ...
    or p_fixture #>> '{participant,display_name}' is distinct from v_expected_display_name
    or p_fixture #>> '{participant,email}' is distinct from v_expected_email
    or p_fixture #>> '{participant,participant_type}' is distinct from 'employee'
    or p_fixture #>> '{participant,addressing_form}' is distinct from v_expected_addressing_form
  then
    raise exception 'GD_FIXTURE_INVALID: participant contract does not match the locked candidate identity.';
  end if;
  ...
  select pg_catalog.count(distinct test_row.value ->> 'test_slug') into v_count
  from pg_catalog.jsonb_array_elements(p_fixture -> 'tests') as test_row(value);
  if v_count <> 3 then
    raise exception 'GD_FIXTURE_INVALID: duplicate standard battery test entries are not allowed.';
  end if;
  ...
  where test.slug = 'ipip-neo-120-v1'
    and test.status = 'active'
    and test.is_active = true
    and question.is_active = true
    and question.is_required = true;
  if v_count <> 120 then
    raise exception 'GD_FIXTURE_INVALID: active IPIP question contract requires 120 questions.';
  end if;
  select pg_catalog.count(*) into v_count
  from public.questions question
  join public.tests test on test.id = question.test_id
  where test.slug = 'safran_v1'
    and test.status = 'active'
    and test.is_active = true
    and question.is_active = true
    and question.is_required = true;
  if v_count <> 45 then
    raise exception 'GD_FIXTURE_INVALID: active SAFRAN question contract requires 45 questions.';
  end if;
  select pg_catalog.count(*) into v_count
  from public.questions question
  join public.tests test on test.id = question.test_id
  where test.slug = 'mwms_v1'
    and test.status = 'active'
    and test.is_active = true
    and question.is_active = true
    and question.is_required = true;
  if v_count <> 19 then
    raise exception 'GD_FIXTURE_INVALID: active MWMS question contract requires 19 questions.';
  end if;
  ...
  -- Resolve and validate every response before the first INSERT. The
  -- response contract has no candidate field, so mixed-candidate payloads
  -- cannot be smuggled through a second candidate envelope.
  for v_response in
    select value
  ...
  loop
    ...
    then
      raise exception 'GD_FIXTURE_INVALID: response contains forbidden candidate, score, report, or identifier fields.';
    end if;
    ...
    then
      raise exception 'GD_FIXTURE_INVALID: response has an invalid standard battery test, question, or response kind.';
    end if;
    ...
    if v_response_kind = 'single_choice' then
      ...
      then
        raise exception 'GD_FIXTURE_INVALID: single_choice requires answer_option_code and null answer_value.';
      end if;
      select pg_catalog.count(*) into v_count
  ...
  where pg_catalog.lower(participant.email) = v_expected_email
    and participant.organization_id <> v_organization_id;
  if v_count <> 0 then
    raise exception 'GD_FIXTURE_INVALID: candidate email already exists in another organization.';
  end if;

  select pg_catalog.count(*) into v_count
  from public.participants participant
  where participant.organization_id = v_organization_id
    and pg_catalog.lower(participant.email) = v_expected_email;
  if v_count > 1 then
    raise exception 'GD_FIXTURE_INVALID: candidate email resolves to multiple Partner Plus participants.';
    ...
    select participant.id,
      ...
      into strict v_participant_id,
                  v_existing_user_id,
                  v_existing_email,
                  v_existing_full_name,
                  v_existing_participant_type,
                  v_existing_status,
                  v_existing_addressing_form
    from public.participants participant
    where participant.organization_id = v_organization_id
      and pg_catalog.lower(participant.email) = v_expected_email;
  end if;

  -- GD-002 through GD-005 must already exist; GD-001 may be created by this generic seam
  -- when no exact participant exists. Existing identity is never taken from
  -- the payload.
  if v_participant_id is not null then
    if v_existing_user_id is not null
      or pg_catalog.lower(v_existing_email) is distinct from v_expected_email
      or v_existing_full_name is distinct from v_expected_display_name
      or v_existing_participant_type is distinct from 'employee'
      or v_existing_status is distinct from 'active'
      or (
        v_existing_addressing_form is not null
        and v_existing_addressing_form is distinct from v_expected_addressing_form
      )
    then
      raise exception 'GD_FIXTURE_INVALID: existing participant identity does not match the locked candidate contract.';
    end if;
  elsif v_candidate_id in ('GD-002', 'GD-003', 'GD-004', 'GD-005') then
    raise exception 'GD_FIXTURE_INVALID: % requires an existing canonical participant.', v_candidate_id;
  end if;
  ...
  if v_participant_id is not null then
    ...
    where assignment.organization_id = v_organization_id
      and assignment.participant_id = v_participant_id
      and assignment.assignment_type = 'standard_battery';
    if v_count <> 0 then
      raise exception 'GD_FIXTURE_NOT_EMPTY: standard battery assignment already exists for %.', v_candidate_id;
    end if;
    ...
    where attempt.organization_id = v_organization_id
      ...
      and test.slug in ('ipip-neo-120-v1', 'safran_v1', 'mwms_v1');
    if v_count <> 0 then
      raise exception 'GD_FIXTURE_NOT_EMPTY: standard battery attempt already exists for %.', v_candidate_id;
    end if;
  end if;
  ...
  where attempt.organization_id = v_organization_id
    and attempt.participant_id = v_participant_id
    and attempt.id in (v_ipip_attempt_id, v_safran_attempt_id, v_mwms_attempt_id);
  if v_count <> 3 then raise exception 'GD_FIXTURE_VERIFY_FAILED: standard battery attempt count.'; end if;
  ...
  return pg_catalog.jsonb_build_object(
    'rpcVersion', 'create_golden_demo_standard_battery_fixture_v2',
    'stateBefore', 'EMPTY',
    'stateAfter', 'CREATED',
    'candidateId', v_candidate_id,
    'participantCreated', v_participant_created,
...
revoke all on function public.create_golden_demo_standard_battery_fixture_v2(jsonb) from public;
revoke all on function public.create_golden_demo_standard_battery_fixture_v2(jsonb) from anon;
revoke all on function public.create_golden_demo_standard_battery_fixture_v2(jsonb) from authenticated;
grant execute on function public.create_golden_demo_standard_battery_fixture_v2(jsonb) to service_role;
