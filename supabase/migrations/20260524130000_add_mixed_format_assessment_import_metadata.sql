...
alter table public.tests
  drop constraint if exists tests_scoring_method_check;

alter table public.tests
  add constraint tests_scoring_method_check
    check (scoring_method in ('likert_sum', 'correct_answers', 'weighted_correct', 'mixed_v1'));
...
declare
  ...
  v_import_strategy jsonb := coalesce(p_package -> 'import_strategy', '{}'::jsonb);
  v_test_metadata jsonb;
  v_dimension_metadata jsonb;
  v_item_metadata jsonb;
  v_option_metadata jsonb;
  v_item_option_catalog jsonb;
...
begin
  ...
  v_test_metadata := jsonb_strip_nulls(
    coalesce(v_test -> 'metadata', '{}'::jsonb)
      || jsonb_build_object(
        'version', nullif(trim(v_test ->> 'version'), ''),
        'intended_use', nullif(trim(v_test ->> 'intended_use'), ''),
        'report_family', nullif(trim(v_test ->> 'report_family'), ''),
        'default_locale', nullif(trim(v_test ->> 'default_locale'), ''),
        'supported_locales', coalesce(v_test -> 'supported_locales', '[]'::jsonb),
        'content_spec', coalesce(p_package -> 'content_spec', 'null'::jsonb),
        'import_mode', coalesce(v_import_strategy ->> 'mode', 'legacy_shared_options_v1')
      )
  );
  ...
  loop
    v_dimension_metadata := coalesce(v_dimension -> 'metadata', '{}'::jsonb);
  ...
  loop
    ...
    if coalesce(v_import_strategy ->> 'mode', '') = 'mixed_format_content_spec_v1' then
      v_item_metadata := jsonb_strip_nulls(
        v_item_metadata || jsonb_build_object(
          'response_format', case
            when v_item_metadata ->> 'response_format' is not null then v_item_metadata ->> 'response_format'
            when v_item ->> 'question_type' = 'single_choice' then 'single_select_likert'
            else null
          end
        )
      );
    ...
    v_item_option_catalog := coalesce(
      v_import_strategy -> 'item_option_catalogs' -> (v_item ->> 'code'),
      p_package -> 'options',
      '[]'::jsonb
    );

    if jsonb_typeof(coalesce(v_item_option_catalog, '[]'::jsonb)) <> 'array' then
      raise exception 'Item % option catalog must be a JSON array.', coalesce(v_item ->> 'code', '<unknown>');
    end if;

    delete from public.answer_option_localizations localization
    using public.answer_options option
    where option.question_id = v_question_id
      and localization.answer_option_id = option.id
      and not exists (
        select 1
        from jsonb_array_elements(coalesce(v_item_option_catalog, '[]'::jsonb)) current_option
        where option.option_order = (current_option ->> 'option_order')::integer
      );
    ...
    for v_option in
      select value
      from jsonb_array_elements(coalesce(v_item_option_catalog, '[]'::jsonb))
    loop
      v_option_metadata := coalesce(v_option -> 'metadata', '{}'::jsonb);
      ...
      values (
        ...
        v_option ->> 'label',
        case
          when jsonb_typeof(v_option -> 'value') = 'number' then (v_option ->> 'value')::integer
          when nullif(trim(v_option ->> 'value'), '') is not null then (v_option ->> 'value')::integer
          else null
        end,
        (v_option ->> 'option_order')::integer,
        case
          when jsonb_typeof(v_option -> 'is_correct') = 'boolean' then (v_option ->> 'is_correct')::boolean
          else null
        end,
        v_option_metadata
      )
