...
export const DASHBOARD_MAIN_CLASS_NAME = "w-full max-w-full px-6 pb-14 pt-24 lg:px-12";

export const DASHBOARD_CONTENT_GRID_CLASS_NAME =
  "grid grid-cols-1 gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(0,2fr)] lg:items-start";

export const DASHBOARD_SIDEBAR_CLASS_NAME = "lg:h-full lg:border-r lg:border-slate-300/80 lg:pr-12";

export const DASHBOARD_SIDEBAR_STACK_CLASS_NAME = "w-full space-y-5";

export const DASHBOARD_PRIMARY_COLUMN_CLASS_NAME = "min-w-0 w-full";

export const DASHBOARD_PRIMARY_COLUMN_STACK_CLASS_NAME = "space-y-6";
  ...
  return (
    <div className={joinClassNames("flex flex-col gap-1.5", className)}>
      ...
      {description ? (
        <p
          className={joinClassNames(
            "font-body text-[15px] leading-7 text-slate-700",
            descriptionClassName,
          )}
        >
          {description}
...
export function DashboardCompactMetaRow({
  ...
  return (
    <div
      className={joinClassNames(
        "mb-4 mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 border-t border-slate-200 pt-3",
        className,
      )}
    >
      {children}
